...
const Sidebar = () => {
  ...
  return (
    <div className="w-64 h-screen glass-card rounded-none border-y-0 border-l-0 flex flex-col p-4 fixed z-40 bg-darkBg">
...
const Layout = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-darkBg text-textColor transition-colors duration-300">
      <Sidebar />
      <main className={`flex-1 transition-all duration-300 ${!isHome ? 'ml-64 p-8' : ''} w-full overflow-x-hidden`}>
        {children}
      </main>
    </div>
  );
